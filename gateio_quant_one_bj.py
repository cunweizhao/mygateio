...
def fetch_candles_window(symbol:str, interval:str, t_from:int, t_to:int) -> List[Dict]:
    ...
    同时遵守 Gate 对“最多只允许距当前 <= 10000 根”的限制。
    """
    out=[]; sec_bar=interval_to_seconds(interval)
    # Gate 限制：最早起点不得超出 to 时刻的 10000 根之前（近似理解为：只允许近 10000 根）
    # 同时很多交易所也把更久远的细分周期做了裁剪。
    min_from_allowed = t_to - 10000*sec_bar + sec_bar
    if t_from < min_from_allowed:
        # 自动截断并提示
        print(f"[提示] Gate 限制最多 10000 根 {interval}，已自动把起点从 {t_from} 调整为 {min_from_allowed} (仅抓取近约 {int(10000*sec_bar)} 秒)")
        t_from = min_from_allowed
    cur=t_from
    # 防止过大窗口导致长时间请求
